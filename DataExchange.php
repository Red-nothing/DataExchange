...
class DataExchange extends Backend
{
	...
	protected function getFieldResults(Database_Result $objConfig)
	{
		...
		$objFields = $this->Database
			->prepare("SELECT * FROM tl_dataexchange_fields WHERE pid=? AND enabled=1 ORDER BY sorting")
		...
		$objResult = $this->Database
			->prepare("SELECT ".implode(', ',$arrQuery)." FROM ".$objConfig->tableName.(empty($arrWhere) ? '' : ' WHERE '.implode(' AND ',$arrWhere)).$strOrderBy)
			->execute($arrValues);
	...
	public function formatValue($strTable,$strField,$varValue)
	{
		...
		elseif($varValue != '' && ($GLOBALS['TL_DCA'][$strTable]['fields'][$strField]['eval']['rgxp'] == 'datim' || in_array($GLOBALS['TL_DCA'][$strTable]['fields'][$strField]['flag'],array(
																																															 5,
																																															 6,
																																															 7,
																																															 8,
	...
	{
		$arrOperations = array();
		$objDBExport = $this->Database
			->prepare("SELECT * FROM tl_dataexchange_config WHERE tableName=? AND addExportInDCA='1'")
			->execute($strName);
