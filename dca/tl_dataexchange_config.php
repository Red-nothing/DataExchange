...
(
	...
	(
		...
		(
			...
			'inputType'                => 'select',
			'options'                  => array(
				','  => $GLOBALS['TL_LANG']['MSC']['comma'],
				';'  => $GLOBALS['TL_LANG']['MSC']['semicolon'],
				'tab'=> $GLOBALS['TL_LANG']['MSC']['tabulator']
			),
			'eval'                     => array(
				'mandatory'=> true,
				'tl_class' => 'w50'
...
class tl_dataexchange_config extends Backend
{
	...
	public function getRowLabel($row,$label,$dc)
	{
		...
		$objFields = $this->Database
			->prepare("SELECT dcaField FROM tl_dataexchange_fields WHERE pid=? AND enabled=1 ORDER BY sorting")
			->execute($row['id']);
...
<div class="limit_height'.(!$GLOBALS['TL_CONFIG']['doNotCollapse'] ? ' h32' : '').' block">
	...
	public function onSubmitCallback(DataContainer $dc)
	{
		...
		if($this->Database->tableExists($strTableName))
		{
			...
			{
				$objFieldExists = $this->Database
					->prepare("SELECT * FROM tl_dataexchange_fields WHERE pid=? AND dcaField=?")
					->execute($dc->activeRecord->id,$tableField['name']);
